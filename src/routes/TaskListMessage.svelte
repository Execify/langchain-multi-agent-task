<script lang="ts">
    export let tasks: Array<{ id: number, task: string, completed: boolean, notes: string }>;
</script>
                ...
                <tbody>
                    {#each tasks as task}
                        <tr>
                            ...
                            <td class="border-t py-2">{task.completed ? 'Yes' : 'No'}</td>
                            <td class="border-t py-2">{task.notes}</td>
                        </tr>
                    {/each}
                </tbody>
            </table>
